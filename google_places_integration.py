...
class GooglePlacesHealthcareCollector:
    def __init__(self):
        config_path = os.path.join(os.path.dirname(__file__), 'config', '.env')  # Adjusted to one level up from script
    ...
    def search_healthcare_providers(self, query, location="Japan", radius=50000):
        ...
        if response.status_code == 200:
            ...
            print(f"Parsed Results for {query}: {results}")  # Debug: Parsed results
    ...
    def search_and_collect_providers(self, search_queries, max_per_query=20):
        """Search and collect comprehensive provider data"""
        all_providers = []
        print(f"Starting collection with {len(search_queries)} queries")
        for i, query in enumerate(search_queries, 1):
            print(f"🔍 Searching {i}/{len(search_queries)}: {query}")
            search_results = self.search_healthcare_providers(query)
            print(f"Search Results for {query}: {len(search_results)} results - {search_results[:2] if search_results else 'None'}")  # Debug: Summary
            if not search_results:
                print(f"⚠️ No search results for {query}")
                continue
            for j, result in enumerate(search_results[:max_per_query], 1):
                print(f"Processing result {j}/{len(search_results[:max_per_query])} for {query}: {result.get('name', 'Unknown')}")
                place_id = result.get('place_id')
                if place_id:
                    print(f"📋 Processing place_id: {place_id}")
                    place_details = self.get_place_details(place_id)
                    print(f"Place Details for {place_id}: {place_details.get('name', 'None') if place_details else 'None'}")
                    if place_details and place_details.get('name'):
                        try:
                            provider_record = self.create_comprehensive_provider_record(place_details)
                            if provider_record and 'provider_name' in provider_record:
                                all_providers.append(provider_record)
                                print(f"✅ Collected: {provider_record['provider_name']}")
                                print(f"   English Proficiency: {provider_record['english_proficiency']}")
                                print(f"   City: {provider_record['city']}")
                                print(f"   Nearest Station: {provider_record.get('nearest_station', 'Not available')}")
                            else:
                                print(f"⚠️ Invalid provider record created: {provider_record}")
                        except Exception as e:
                            print(f"❌ Error creating provider record: {str(e)}")
                    else:
                        print(f"⚠️ No valid details for {result.get('name', 'Unknown')}")
                else:
                    print(f"⚠️ No place_id for {result.get('name', 'Unknown')}")
                time.sleep(0.1)
            time.sleep(1)
