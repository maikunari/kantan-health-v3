...
const ContentGeneration: React.FC = () => {
  ...
  const handleStartGeneration = async () => {
    try {
      ...
      await api.post(API_ENDPOINTS.CONTENT_GENERATE, {
      ...
      message.success('Content generation started successfully');
      setGenerating(true);
      fetchStatus();
    } catch (error: any) {
      message.error(error.response?.data?.error || 'Failed to start content generation');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <Card title="Generation Control Panel">
        {status.batch_running && (
          <Alert
            ...
            description="The AI batch processor is currently generating content. New requests will be queued."
            type="info"
            showIcon
      ...
      >
        <div style={{ color: '#666' }}>
          <Text type="secondary">
            Content generation activity and logs will appear here...
          </Text>
        </div>
      </Card>
    </div>
